...
control IngressPipeImpl (inout parsed_headers_t    hdr,
    ...
    table ndp_match_MAC_table {
        ...
        @name("ndp_match_MAC_table_counter")
        counters = direct_counter(CounterType.packets_and_bytes);
    }
    //
    ...
    table L2_station_table {
        ...
        @name("L2_station_table_counter")
        counters = direct_counter(CounterType.packets_and_bytes);

    }
    //
    // 3. Create a table for IPv6 routing. An action selector should be used to
    ...
    action set_next_hop(mac_addr_t dmac) {
        ...
        hdr.ethernet.dst_addr = dmac; //dst mac
        // Decrement TTL
    ...
    table ipv6_routing_table {
        key = {
            ...
            // Following fields are optional per PFC6438
    ...
    // Process the packet if the destination IP is the segment Id(sid) of this device.
    // The table will then decrement the "segment left" field from the Srv6 header and set destination IP 
    // address to the next segment
    ...
    //This action is called directly in the apply block
    action srv6_pop(){
        ...
        // Now set MAX_HOPS headers invalid
    ...
    apply {
        ...
        if (hdr.icmpv6.isValid() && hdr.icmpv6.type == ICMP6_TYPE_NS) {
            ...
            if (ndp_match_MAC_table.apply().hit){
                do_l3_l2 = false;
            }
        }
        ...
        if (do_l3_l2) {
            ...
            //if (hdr.ipv6.isValid() && L2_station_table.apply().hit){
            ...
            if (hdr.ipv6.isValid() && L2_station_table.apply().hit){
                ...
                if (srv6_my_sid.apply().hit){
                    //Penultimate Segment Pop (PSP) logic - enabled for all packets
